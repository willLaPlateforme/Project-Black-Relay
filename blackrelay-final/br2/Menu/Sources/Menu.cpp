...
// Helper : dessine le background de la fenêtre 800x600
    ...
    : _window(window), _selectedIndex(0), _activatedIndex(-1), _animTimer(0.f)
{
    ...
    _items = {{"CONTINUER"}, {"NOUVELLE PARTIE"}, {"SCORES"},
              {"PARAMETRES"}, {"CREDITS"}, {"QUITTER"}};
...
void Menu::_drawNewsPanel() {
    sf::Text tag(_font);
    tag.setCharacterSize(10);
    tag.setFillColor(sf::Color(140, 140, 140));
    tag.setString("// NEWS_FEED");
    tag.setPosition({12.f, 480.f});
    _window.draw(tag);

    sf::Text alert(_font);
    alert.setCharacterSize(11);
    alert.setFillColor(sf::Color(220, 30, 30));
    alert.setStyle(sf::Text::Bold);
    alert.setString("ALERTE GLOBALE");
    alert.setPosition({16.f, 500.f});
    _window.draw(alert);

    sf::Text news(_font);
    news.setCharacterSize(10);
    news.setFillColor(sf::Color(140, 140, 140));
    news.setString("Activite ennemie en hausse.\nRestez vigilants, Commandant.");
    news.setPosition({16.f, 518.f});
    _window.draw(news);
}
...
void Menu::_drawCornerDeco() {
    auto corner = [&](float x, float y, float sx, float sy) {
        ...
        h.setFillColor(sf::Color(220, 30, 30, 100));
        h.setPosition({x, y});
        h.setScale({sx, 1.f});
        _window.draw(h);
        sf::RectangleShape v({1.f, 18.f});
        v.setFillColor(sf::Color(220, 30, 30, 100));
        v.setPosition({x, y});
        v.setScale({1.f, sy});
        _window.draw(v);
    };
    corner(2.f,   2.f,   1.f,  1.f);
    corner(798.f, 2.f,  -1.f,  1.f);
    corner(2.f,   598.f, 1.f, -1.f);
    corner(798.f, 598.f,-1.f, -1.f);
}
