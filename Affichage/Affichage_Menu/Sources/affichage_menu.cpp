...
#include "../../Affichage_Options/Includes/Affichage_Options.hpp"
#include <cmath>
...
Affichage_Menu::Affichage_Menu(sf::RenderWindow& window)
    : _window(&window), _menuState(MenuState::MAIN),
      ...
      _animTimer(0.f), _scores(window, _font)
{
    ...
    _newsPanel.setPosition({8.f, 628.f});
}

void Affichage_Menu::rebind(sf::RenderWindow& window) {
    _window = &window;
    _scores.rebind(window);
    _play = false;
    _quit = false;
}

void Affichage_Menu::saveScore(const std::string& name, int score) {
    _scores.addScore(name, score);
}
...
void Affichage_Menu::_drawNewsPanel() {
    ...
    tag.setPosition({12.f, 614.f}); _window->draw(tag);
    sf::Text alert(_font); alert.setCharacterSize(13);
    alert.setFillColor(sf::Color(220,30,30)); alert.setStyle(sf::Text::Bold);
    alert.setString("ALERTE GLOBALE"); alert.setPosition({76.f, 632.f}); _window->draw(alert);
    ...
    news.setPosition({76.f, 648.f}); _window->draw(news);
}
